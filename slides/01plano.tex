...
% tres provas > 5
	...
	% 	\end{itemize}
		\item A avaliação será constituída por cinco provas escritas, atividades semanais realizadas durante e após as aulas e atividades realizadas em laboratório.
	% 	\item A média final ($M_{final}$) será calculada de seguinte maneira: 
	% 	\begin{itemize}
			\item Se $M_{provas} < 5$ então $M_{final} = minimo(M_{provas}; M_{disciplina})$
            \item Senão $M_{final} = M_{disciplina} = M_{provas}\times 0,6 + M_{praticas}\times 0,2 + M_{exerc.grupo} \times 0,15+ M_{quest.} \times 0,05 $
                \item $M_{praticas} = M_{simulacoes} \times 0,5 + M_{laboratorios} \times 0,5$
		\end{itemize}
		\item Será aprovado o aluno que obtiver média final igual ou superior a 6. 
		\begin{itemize}
			\item Observação: Não haverá provas substitutivas. 
	...
	\framebreak
        \item Simulações (10\%):
		\begin{itemize}
		    \item objetivo: preparação para as aulas práticas;
		     \item prazo: 00:00 de segunda-feira;
		    \item tentativas infinitas;
		    \item desempenho: último envio.
		\end{itemize}
		
		\item Laboratórios na Bancada (10\%):
		\begin{itemize}
		    \item consolidação da teoria (estar preparado);
		    \item participação ativa;
		    \item oportunidade de estudo, fixação e esclarecimentos;
		    \item panorama de desempenho pessoal.
		\end{itemize}
